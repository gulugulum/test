...
#define  LOAD_FIRMWARE_SIZE_MAX   0x80000
...
void help(void)
{
    printf("usage: libusb-test [-h] [-b] [-i] [-v vid] [-p pid] [-f file_path] [-l] [-a] \n");
...
static char * uboot_file = "/home/guluya/test_usblib/t31/uboot.bin";  //
...
int read_data_to_buf(unsigned char *data_buf, const char *filename)
{
    ...
    if (fp != NULL) {
        size_t len = file_size_get(filename);
...
int  file_load( const char *filename)
{
           ...
           uint8_t *buf = NULL;
              int ret = 0, rlen = 0;
            buf = (uint8_t *)malloc(LOAD_FIRMWARE_SIZE_MAX*2);
            if (buf) 
            {
                 rlen = read_data_to_buf(buf,filename);
                if ((rlen > 0) && (rlen < LOAD_FIRMWARE_SIZE_MAX)) 
                {
                                  printf("load  file %d bytes.\n", rlen);
                                bulk_send(buf, rlen);
                }
                else       
                {   printf("Error:%s FPGA firmware too large %d\n",__func__,rlen);
                 ret = RET_EINVAL;
             }
            }
               free(buf);
...
{  
   ...
   r = pthread_create(&usb_monitor_thread_id, 0, usb_monitor_thread, 0); 
    ...
    while(1)
    {
        ...
        else   if(strncmp(send_buf, "loaduboot", 9) == 0){
                            file_load(uboot_file);
